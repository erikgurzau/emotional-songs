...
public class APIService implements APIConfig {
    ...
    public API getAPIByNome(String nome) {
        Optional<API> resultAPI = Arrays.stream(apis).filter(api -> api.getNome().equals(nome)).findFirst();
        return resultAPI.orElse(null);
    }

    @Override
    public API[] getAPIByCodGruppo(String codGruppo) {
        return (API[]) Arrays.stream(apis).filter(api -> api.getCodiceGruppo().equals(codGruppo)).toArray();
    }
}
